...
def update_data_files() -> bool:
    ...
    for key in config["bel_resources"]["openbel"]["openbel_namespace_sources"]:
        ...
        (changed_flag, msg) = utils.get_web_file(url, local_fn, days_old=update_cycle_days)
        log.info(msg)
...
def build_json(nsfiles):
    ...
    for nsfile, nsfile_src_url in nsfiles:
        ...
        for idx in range(idx_len):
            ...
            terms_filename = f'{config["bel_resources"]["file_locations"]["data"]}/namespaces/{namespace}_belns.jsonl.gz'
